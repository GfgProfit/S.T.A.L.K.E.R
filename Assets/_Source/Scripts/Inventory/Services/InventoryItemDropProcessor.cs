...
{
    private readonly Func<ItemData, int, float, bool> _spawnWorldItem;
    ...
    public InventoryItemDropProcessor(Func<ItemData, int, float, bool> spawnWorldItem, Func<InventoryGrid, InventoryItem, bool> detachItemFromGrid, Action<InventoryItem> destroyInventoryItem, Action refreshWeightState)
    {
    ...
    public bool TryDropItem(InventoryGrid grid, InventoryItem item, bool wholeStack)
    {
        ...
        {
            if (_spawnWorldItem(itemData, amountToDrop, durabilityPercent) == false)
            {
                return false;
            }
