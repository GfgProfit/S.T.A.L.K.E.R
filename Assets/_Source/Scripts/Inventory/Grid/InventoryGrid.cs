...
public abstract class InventoryGrid : MonoBehaviour
{
    ...
    public virtual Vector2 GetHighlightSize(InventoryItem inventoryItem, int posX, int posY) => new(inventoryItem.Width * ItemGrid.TILE_SIZE_WIDTH, inventoryItem.Height * ItemGrid.TILE_SIZE_HEIGHT);
    public virtual Vector2 GetHighlightPosition(InventoryItem inventoryItem, int posX, int posY) => CalculatePositionOnGrid(inventoryItem, posX, posY);
    public virtual bool CanMergeStackAt(int posX, int posY) => true;
